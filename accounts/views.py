...
def custom_admin_login(request):
    form = CustomAdminAuthenticationForm(request, data=request.POST or None)
    print("The paht is : ",request.path)
    if request.method == "POST":
        ...
        if user is not None:
            if getattr(user, "is_2fa_enabled", False):
                # Step 2: If 2FA enabled but no TOTP entered yet → show TOTP form
                if not totp_code:
                    form = CustomAdminAuthenticationForm()
                    form.fields["username"].initial = username
                    form.fields["username"].widget.attrs["readonly"] = True
                    form.fields["password"].widget = forms.HiddenInput(attrs={'hidden':True})
                    form.fields["totp_code"].required = True
                    form.fields["password"].initial = password
                    # messages.info(request, _("Please enter your 2FA code."))
                    return render(request, "admin/login.html", {"form": form, "totp_stage": True})
            ...
            # return redirect(admin_site.index_path)
            return redirect(request.GET.get("next") if request.GET.get("next") else '/admin/')
        else:
